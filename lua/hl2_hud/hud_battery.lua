...
-- Visible only when Armor() > 0. Fades out on SuitPowerZero.
-- Animation events: SuitPowerIncreasedAbove20, SuitPowerIncreasedBelow20,
--                   SuitDamageTaken, SuitPowerZero
...
local function event(name)
    ...
    if name == "SuitPowerIncreasedAbove20" then
        ...
        set(state.textColor, C.FgColor,  "Linear",  0,    0.05)
...
function elem:Draw(x, y, clip_h)
    ...
    step(state.fgColor) step(state.textColor) step(state.bgColor)
    step(state.blur)    step(state.alpha)
    ...
    local fs = {
        ...
        textColor = { cur = fa(state.textColor.cur) },
