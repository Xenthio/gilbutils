...
--            digit2_xpos=98 digit2_ypos=16  (reserve count, small font)
-- Shows clip ammo (large) + reserve (small). Hidden when no weapon / no primary ammo.
...
function elem:Draw(x, y, clip_h)
    ...
    local reserve
    if clip < 0 then
        -- No clip: total ammo shown as clip number, no reserve
    ...
    -- Label "AMMO" (text_xpos=8, text_ypos=20) — fgColor per CHudNumericDisplay::PaintLabel
    surface.SetFont("HL2Hud_Text")
    surface.SetTextColor(state.fgColor.cur)
    surface.SetTextPos(x + 8*s, y + 20*s)
    surface.DrawText("AMMO")

    -- Clip count (large, digit_xpos=40, digit_ypos=2)
