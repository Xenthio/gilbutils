...
local GIB_GRAVITY    = 800        -- used to set entity gravity scale
...
function ENT:Initialize()
	...
	-- Engine handles gravity + integration. Touch() handles collision response.
	...
	-- Scale gravity to match our 800 u/s² constant (default sv_gravity is 600)
	...
	-- Spin via engine angle velocity
	self:SetLocalAngularVelocity(Angle(math.Rand(100, 200), math.Rand(100, 300), 0))

	-- GibVelocity set by SpawnGib helper; apply it now
...
function ENT:Touch(ent)
	...
	local tr     = self:GetGroundEntity()  -- cheapest ground check available
	local normal = Vector(0, 0, 1)

	-- Get a better normal from a short downward trace
	local downTr = util.TraceLine({
		start  = self:GetPos(),
		endpos = self:GetPos() + vel:GetNormalized() * 2,
		filter = self,
		mask   = MASK_SOLID,
	})
	if downTr.Hit then
		normal = downTr.HitNormal
	end
	...
	if self.BloodDecalsLeft > 0 and self.BloodColor ~= DONT_BLEED then
		...
		util.Decal(decal, hitPos + normal, hitPos - normal)
	...
	-- Apply our HL1 ClipVelocity response
	local newVel = ClipVelocity(vel, normal, GIB_ELASTICITY)

	if isFloor and math.abs(newVel.z) < 60 then
		-- Settle onto ground: kill vertical component, switch to friction Think
		newVel.z = 0
		...
		ang.p = 0; ang.r = 0
		self:SetAngles(ang)
		-- Switch to MOVETYPE_WALK so engine keeps us on the ground surface
...
function ENT:Think()
	...
	-- Lifetime / settle check (still airborne)
	if not self.GibOnGround then
		if CurTime() > self.WaitTillLandTime then
			local speed = self:GetAbsVelocity():LengthSqr()
			if speed < 1 then
	...
	-- Grounded: apply friction manually each tick
	local vel   = self:GetAbsVelocity()
	local dt    = FrameTime()
